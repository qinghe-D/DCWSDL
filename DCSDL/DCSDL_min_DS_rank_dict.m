...
% Choose a rho.
% Algorithm summary
% ADMM: D,J = argmin_D (1+¦Á/2)trace(QD'D) - trace(SD') + ¦Ã||J||_*
% s.t ||d_i||_2^2 <= 1 and J = D
% Alternatively solving:
% (1): D^{k+1} = argmin_D (1+¦Á/2)trace(QD'D) - trace(SD') + rho/2 ||J - D + U^k||_F^2 
% 		s.t. ||d_i||_2^2 <= 1
...
% -----------------------------------------------
    ...
    if lambdaD == 0
        D = E * pinv(F);
%     	D = ODL_DCSDL_updateDS(Dinit, E, F, opts);
        if nargin == 0
            D = [];
        end 
    	return;        
	...
	% s.t. ||d_i||_2^2 <= 1
    % D = argmin_D (1+¦Á/2)trace(QD'D) - trace(SD') + rho/2 ||J - D + U||_F^2 
	% s.t. ||d_i||_2^2 <= 1
	E1 = E + rho/2*(J_old + U_old);
	F1 = F + rho/2*I;
    D_new = E1 * pinv(F1);
% 	D_new = ODL_DCSDL_updateDS(D_old, E1, F1, optsD);
